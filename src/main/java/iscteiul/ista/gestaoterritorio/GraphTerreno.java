...
public class GraphTerreno {
    ...
    /**
     * Setter para a classe GraphTerreno.
     *
     * @param newGraph novo grafo.
     */
    public void setGraph(SimpleGraph<String, DefaultEdge> newGraph) {
        graph.vertexSet().clear(); // Remove todos os vértices existentes
        graph.edgeSet().clear(); // Remove todas as arestas existentes

        if (newGraph != null) {
            // Adiciona os vértices do novo grafo
            newGraph.vertexSet().forEach(graph::addVertex);
            // Adiciona as arestas do novo grafo
            newGraph.edgeSet().forEach(edge -> {
                String source = newGraph.getEdgeSource(edge);
                String target = newGraph.getEdgeTarget(edge);
